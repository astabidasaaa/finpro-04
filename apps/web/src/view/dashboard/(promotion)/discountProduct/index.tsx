...
import DiscountProductPromotionTable from './DiscountProductPromotionTable';
...
export default function DiscountProductPromotionView() {
  ...
  const [promotions, setPromotions] = useState<ProductDiscountPromotionProps[]>(
    [],
  );
  ...
  async function fetchData() {
    try {
      ...
      router.push(`/dashboard/promotion/discount-product?${params.toString()}`);
      ...
      if (user.role === UserType.STOREADMIN) {
        ...
        const promotionResult = await axiosInstance().get(
          `${process.env.API_URL}/promotions/discountproduct?${params.toString()}&storeId=${storeId}`,
          {
            headers: {
              'Content-Type': 'application/json',
              Authorization: `Bearer ${token}`,
      ...
      if (user.role === UserType.SUPERADMIN) {
        const promotionResult = await axiosInstance().get(
          `${process.env.API_URL}/promotions/discountproduct?${params.toString()}`,
          {
            headers: {
              'Content-Type': 'application/json',
              Authorization: `Bearer ${token}`,
      ...
      <div className="w-full">
        ...
        >
          ...
          <TabsContent value={promotionState}>
            <DiscountProductPromotionTable data={promotions} />
