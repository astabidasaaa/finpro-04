...
import FreeProductPromotionTable from './FreeProductPromotionTable';
...
export default function FreeProductPromotionView() {
  const { user } = useAppSelector((state) => state.auth);
  const [promotions, setPromotions] = useState<FreeProductPromotionProps[]>([]);
  ...
  async function fetchData() {
    try {
      ...
      router.push(`/dashboard/promotion/free-product?${params.toString()}`);
      ...
      if (user.role === UserType.STOREADMIN) {
        ...
        const promotionResult = await axiosInstance().get(
          `${process.env.API_URL}/promotions/freeproduct?${params.toString()}&storeId=${storeId}`,
          {
            headers: {
              'Content-Type': 'application/json',
              Authorization: `Bearer ${token}`,
      ...
      if (user.role === UserType.SUPERADMIN) {
        const promotionResult = await axiosInstance().get(
          `${process.env.API_URL}/promotions/freeproduct?${params.toString()}`,
          {
            headers: {
              'Content-Type': 'application/json',
              Authorization: `Bearer ${token}`,
      ...
      <div className="w-full">
        ...
        >
          ...
          <TabsContent value={promotionState}>
            <FreeProductPromotionTable data={promotions} />
